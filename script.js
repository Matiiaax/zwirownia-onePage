...
window.addEventListener('resize', () => {
  ...
  // ── LEAFLET MAP ──
  // ── LEAFLET MAP — LAZY LOAD (poprawka wydajności) ──
  // Leaflet JS/CSS (40 KiB, 2700ms blokowania renderowania) wczytuje się
  // dopiero, gdy użytkownik faktycznie zbliża się do sekcji #obszar,
  // a nie zawsze na starcie strony.
    ...
    function initServiceMap() {
      ...
      const resetControl = L.Control.extend({
        ...
        onAdd: function() {
          ...
          btn.innerHTML = '<span style="display:inline-flex;align-items:center;gap:6px;"><span style="font-size:1rem;line-height:1;">⟳</span><span>Resetuj widok</span></span>';
          btn.style.cssText = 'background:#1e1e1e;color:#F5A623;border:1.5px solid #F5A623;padding:7px 14px;border-radius:6px;cursor:pointer;font-family:Oswald,sans-serif;font-size:0.78rem;letter-spacing:1px;text-transform:uppercase;font-weight:700;box-shadow:0 4px 14px rgba(0,0,0,0.4);display:inline-flex;align-items:center;-webkit-tap-highlight-color:transparent;';
          var isTouchDevice = ('ontouchstart' in window);
          if (!isTouchDevice) {
            btn.onmouseover = function() { btn.style.background = '#F5A623'; btn.style.color = '#1e1e1e'; };
            btn.onmouseout = function() { btn.style.background = '#1e1e1e'; btn.style.color = '#F5A623'; };
          }
          L.DomEvent.on(btn, 'click', function(e) {
            L.DomEvent.stopPropagation(e);
            map.fitBounds(podlaskieArea.getBounds(), { padding: [24, 24], animate: true });
          });
          return btn;
        }
      });
new resetControl().addTo(map);
    ...
    let mapLoaded = false;
    const mapSection = document.getElementById('obszar');
    const mapObserver = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && !mapLoaded) {
        mapLoaded = true;
        loadLeafletAssets(initServiceMap);
